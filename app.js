...
function renderHome(){
  ...
  if(photos.length){
    ...
    hero.prepend(img);if(placeholder)placeholder.style.display='none';
  ...
  [[30,'p30'],[90,'p90'],[365,'p365']].forEach(([n,id])=>{const p=clamp(Math.round(days/n*100),0,100),bar=document.getElementById(id),text=document.getElementById(id+'Text');if(bar)bar.style.width=p+'%';if(text)text.textContent=p+'%'});
...
function renderCoachMission(){
 ...
 const days=projectDay(),welcomeDayEl=document.getElementById('welcomeDay'),welcomeMessageEl=document.getElementById('welcomeMessage');if(welcomeDayEl)welcomeDayEl.textContent=`${days}일째`;const greet=['오늘도 미래의 피부를 위한 작은 선택 하나.','오늘의 기록이 내일의 자신감을 만듭니다.','완벽함보다 꾸준함이 더 강합니다.','피부는 조급함보다 시간을 좋아합니다.','오늘도 나를 다정하게 돌보는 하루.'];if(welcomeMessageEl)welcomeMessageEl.textContent=greet[hashDate(today()+'greet')%greet.length];renderJourney(days);
}
